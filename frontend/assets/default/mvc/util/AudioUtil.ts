...
export default class AudioUtil {
    ...
    /**
     * 从音乐对象池内移除对象
     * @param path 音乐路径，例如audioclips/dingding
     */
    public static removeEffectFromPool(path: string): void {
...
class __AudioSource {
    ...
    private loadRes(cb: () => void): void {
        if (!this._loaded) {
            ITResourceLoader.loadRes(this._path, AudioClip, (err: any, res: AudioClip) => {
                if (err) {
    ...
    public destroy(): void {
        this._audioSource.stop();
        this._audioSource.node.destroy();
